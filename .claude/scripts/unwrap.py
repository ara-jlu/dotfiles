...
def unwrap_text(text, joins=None):
    ...
    while i < len(lines):
        ...
        # 段落、またはリスト項目。リスト項目ならマーカーまでを、ただの段落なら行頭のインデントを prefix として外し、本文だけを畳んであとから prefix を戻す。段落のインデントを戻さないと、リスト項目の中の継続段落が桁 0 に出てリストがそこで切れる。リスト項目の数も段落テキストも変わらないので、verify のうち構造の数を数える条件と段落テキストを比べる条件では検出できない。捕まえるのは `_paragraph_indents` の条件である。だから結合の側で必ずインデントを保つ。
