...
def fence_closes(line, char, length, indent):
    ...
    **閉じマーカーのインデントは開きマーカーから 3 桁までである。** それより深くインデントされた行は中身であって、閉じない。絶対の桁数で「3 桁まで」と書いてはならない。リスト項目の中のフェンスは 4 桁以上インデントされて開くことがあり、その閉じマーカーも同じ深さにあるからである。見るのは **開きからの相対**である。
    ...
    以前はマーカーの文字と長さを見ず、`^\\s*(```|~~~)` に当たるどの行でも開閉を反転させていた。そのため 4 個のバッククォートで開いたフェンスの中に 3 個の行があると、そこで反転して**以降のフェンスの内と外が入れ替わり**、本来コードである ```` ```ts ```` ブロックが本文として結合された。verify の 3 条件 (文字の一致・構造の数・段落の頭のインデント) はどれもこの壊れ方を検出できない。
    """
    m = FENCE.match(line)
    if not m:
        return False
...
def verify(before, after):
    ...
    空のリストなら安全である。設計の「検証」の3条件を実装している: 段落テキストが文字単位で一致すること、構造の数が変わらないこと、段落の頭のインデントの並びが変わらないこと。
