...
def indent_width(indent):
    ...
    `len()` で数えてはならない。**フェンスの「開きから 3 桁まで」の判定はタブを展開した桁で行う規則**であり、タブを 1 桁と数えると開きから 4 桁以上の位置にある ``` の行がフェンスを閉じてしまう。そこから先のファイルの内と外が入れ替わり、本来コードである領域が本文として結合される。これはフェンスの入れ子・HTML ブロックに続く **3 件目の、開閉の範囲の取り違え**であり、実データ (joifup の `notes/document/227-*`) で発火していた。**verify の第 1〜4 条件はどれもこの壊れ方を検出できず、第5条件 (AST 比較) だけが捕まえた。**
    """
    width = 0
    for ch in indent:
...
# 足しているのは、**日本語の技術文書の本文に普通に出る記号**である: 一般句読点 (`—` `…` `‐` `※` の U+2010–U+205E)、矢印 (`→` `←`)、丸囲み数字 (`①`)、`§`・`°`・`×`、そして **`/` と `.`**。実測で `——` だけが 181 件出ており、これを未知の形として並べると一覧の先頭が本文の飾りで埋まる。
# **`/` と `.` はノイズの最上位だった。** `~/Joifup` 全体で 999 件 / 495 形のうち `/` が 317 件 (32%) を占め、実体は `` `a` / `b` / `c` `` のような列挙の行末で全部本文だった。足すと 605 件 / 433 形に落ちる。`.` はさらに 88 件 / 71 形を落とし、消えるのは `` `.gitignore` ``・`` `.uat-evidence/<task>/` ``・`` `.focus()` `` のような**記号で始まるコードスパン**と英文の終止符だけで、ブロックのマーカーは 1 件も含まれない (全件を目で確かめた)。
# **足してもブロックの境界は見える。** `-->` は右が `-`、`</div>` は右が `<`、`:::` は `:`、`$$` は `$`、Setext の `===` は `=`、TOML の `+++` は `+` で当たり続ける。どれも `/` や `.` を含まない。
# **`.` は剥がす側 (INLINE_MARKUP) ではなくこちらに足す。** `.` は markdown のインライン記法ではないので、剥がす側に置くと名前が意味と合わない。実測でも落ちる形はこちらのほうが広く、剥がす側だと `` ...)` `` のような形が残る。
