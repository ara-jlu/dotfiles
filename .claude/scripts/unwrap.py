...
# 無いときに黙って飛ばしてはならない。この道具は「黙って成功に見える」形を何度も潰してきた。飛ばしたことは main が最後に警告として出す。`verify` の戻り値からは分からない —— `ast_problems` はパーサが無いときも検査を通ったときも空のリストを返すので、呼び出し側はモジュール変数の `HAS_AST_PARSER` を自分で見るしかない。
try:
...
# 結合点の「普通の文字」。この正規表現が持つのは ASCII の英数字と、設計が定めた記号の範囲だけである。かな・漢字・全角の約物は `JA` が持っていて、`_is_ordinary` がこの正規表現と or で繋ぐ。**どれでもない文字が結合点の左右に現れたら、それは本文の結合ではないかもしれない。**
...
def _is_ordinary(ch):
    """結合点の片側の文字が、日本語か英数字のどちらかである「普通の文字」か。"""
...
def unwrap_text(text, joins=None):
    ...
    while i < len(lines):
        ...
        # 段落、またはリスト項目。リスト項目ならマーカーまでを、ただの段落なら行頭のインデントを prefix として外し、本文だけを畳んであとから prefix を戻す。段落のインデントを戻さないと、リスト項目の中の継続段落が桁 0 に出てリストがそこで切れる。リスト項目の数も段落テキストも変わらないので、verify のうち構造の数を数える条件と段落テキストを比べる条件では検出できない。捕まえるのは `_paragraph_indents` の条件である（AST 比較の条件もこの形を捕まえるが、あちらは任意のパッケージが無ければ丸ごと飛ぶ。必ず発火するのは `_paragraph_indents` のほうである）。だから結合の側で必ずインデントを保つ。
...
# 英数字か CJK (かな・漢字) のどちらかである文字。これを **1 つも含まない非空行**が「マーカーだけの行」である。
