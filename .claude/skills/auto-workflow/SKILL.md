...
このスキルは**ユーザーの入力を待たずに全ステップを自動実行**します。各ステップ完了後、必ず次のステップに進んでください。
...
検証可能な項目がある場合は実行し、結果を報告に含める。検証不可能な項目がある場合は「手動確認推奨」としてリストアップする。
...
**⚠️ この報告は省略禁止**。「ユーザーアクション必要」セクションは、該当ゼロの場合も「なし」と明記すること。このリストはステップ6.4でNotionタスク起票に使用する。
...
**PR作成前に**ユーザーアクション項目を Notion にタスクとして起票する。起票後の Notion URL を 6.5 の PR 本文に含めるため、このステップを先に実行する。

**起票条件**: 以下のいずれかに当てはまる項目があれば起票する。なければスキップして 6.5 へ。
- ステップ2.6の「ユーザーアクション必要」に列挙した項目がある
- ステップ2.6を出力しなかった・または「ユーザーアクション必要」セクションを出力しなかった場合 → 今から実装内容を振り返り、下記基準で能動的に洗い出す

**手動確認が必要と判断する基準**（AI が自動検証できないもの）:
- 実機・実ブラウザ・実デバイス上でのみ確認できる動作
- 認証・権限が必要な外部サービス（管理コンソール、API キー設定等）
- ファイルシステムや OS レベルの操作
...
PRテンプレートに従って本文を生成（詳細は `references/pr-template.md` を参照）。**ステップ6.4で起票したタスクがある場合、Notion URL をリンク付きチェックリストとして含めること。**
