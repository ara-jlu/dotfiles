...
順序を固定しているのは、PR URL が後続の手順から参照される前に取得されるようにするためである。

**絶対に Done にせず、絶対にマージしない。** status→Done と `chore(joifup): approve TASK-xxx` とマージは、人間の承認セッションが持つ。

--dry-run はネットワークと副作用のある手順 (git / gh / curl) を止め、実行するはずのコマンドを出す。
ローカルのファイル操作 (status の書き換え) は、検証できるようにするため両方のモードで走る。
読み取りしかしない手順 (UAT 証跡の事前確認、証跡添付の事前確認、`git rev-parse` によるブランチ名の取得など) も両方のモードで走る —— 何も変えないうえ、その答えこそが dry-run を読む価値だからである。
"""
import argparse
import datetime
import json
