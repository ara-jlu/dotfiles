...
Run: `cd $SCRATCH && python3 unwrap.py <WT>/notes <WT>/tasks <WT>/projects <WT>/.claude`（`<WT>` は worktree の絶対パス）
...
報告には、適用前後の数と、残った箇所の内訳（ファイル名と残った理由）を書く。
