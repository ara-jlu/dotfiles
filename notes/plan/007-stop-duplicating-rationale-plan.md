...
**`MIN_GH_VERSION = (2, 99, 0)` の扱い —— この計画は当初ここを誤っていた。** 「joifup の 295 設計書に `2.99.0` が無い」ことだけを見て「コードにしか無い一次情報だ」と断定したが、**dotfiles 自身の `notes/document/005-align-skills-with-pr-attached-uat-evidence-design.md:32,34` が下限を確定させている**（「`--attach` の初出は v2.99.0（2026-09-01）」「したがって下限は `2.99.0`」）。定数の上の `# 2.99.0 は --attach の初出。` は、その 005 の逐語写しである。

**したがってこれは残すのではなく、005 を指すポインタに畳む。** 値そのものは非テストだけで4箇所（005 設計書・`j-finish/SKILL.md`・`j-pr/SKILL.md`・`j-pr/references/pr-body.md`）にあるので、**どこに在るかを数えて書くこともしない** —— 数え上げは必ず漏れ、漏れた面が陳腐化する。

**教訓（この計画の欠陥として記録する）: 「ここにしか無い」と書く前に、そのリポジトリの `notes/` を grep すること。** 1つの設計書に無いことは、どこにも無いことを意味しない。
