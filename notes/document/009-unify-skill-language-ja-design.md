...
全行が変わる差分なので「訳しただけ」は目視では検査できない。旧版（`git show <base>:<file>`）と新版を比べる不変条件を置き、機械で確認する。設計時に 6 つ置き、実装中に 2 つ追加して**最終的に 8 条件**になった（7 と 8 は、当初の 6 条件では検出できない欠陥が実ファイルで見つかったため足した）。
...
6. `**…**` 強調の**個数**が一致すること
7. **段落行の本数**が一致すること（実装中に追加。マークアップを持たない段落がまるごと消えても 1〜6 は全部 pass するという穴があった）
8. 節ごとの**テーブル行数**が一致すること（実装中に追加。`j-recap` のテーブルから行を削除しても 1〜7 は全部 pass することを実演で確認した）
...
2. 不変条件が 9 ファイル全部で pass すること（検査出力を PR 本文に載せる）。
